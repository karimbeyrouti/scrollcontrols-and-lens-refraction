...
export default function App() {
  return (
    <Canvas gl={{ antialias: false }} camera={{ position: [0, 0, 20], fov: 15 }}>
      ...
      {/* Remove the environment if you don't want reflections on the lens */}
      <Environment files="https://dl.polyhaven.org/file/ph-assets/HDRIs/hdr/1k/thatch_chapel_1k.hdr" />
    </Canvas>
  )
}
...
function Lens({ children, damping = 0.15, ...props }) {
  ...
  useFrame((state, delta) => {
    ...
    // The following code will render that into a buffer, whose texture will then be fed into a simple plane
    // spanning the full screen and the lens transmission material
